...
app.patch("/jokes/:id",(req,res) => {
  ...
  const joke = req.body.joke || existingJoke.jokeText
  const type = req.body.type || existingJoke.jokeType
  const searchIndex = jokes.findIndex(joke => joke.id == id)
  const replacementJoke = {
    id: id,
    jokeText: joke,
...
app.delete("/jokes/:id",(req,res) => {
  ...
  const searchIndex = jokes.findIndex((joke) => joke.id == id)
  if (searchIndex !== -1) {
  jokes.splice(index, 1); // Remove the joke
}
  jokes = jokes.filter(j => j !== null);
  res.json(jokes)
})
...
app.delete("/all",(req,res) =>{
  ...
  if (key == masterKey){
    ...
    res.send("You must specify valid api key")
  }
})
